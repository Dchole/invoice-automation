...
def export_backup():
    ...
    return FileResponse(str(db_path), filename="invoices_backup.db", media_type="application/octet-stream")
...
def export_invoices_csv(db: DbSession = Depends(get_db)):
    ...
    writer.writerow(["Invoice Number", "Client", "Issue Date", "Due Date", "Subtotal", "Tax Rate %",
                     "Tax Amount", "Total", "Amount Paid", "Balance", "Currency", "Status", "Sent Date", "Paid Date"])
    for inv in invoices:
        client = db.get(Client, inv.client_id)
        balance = float(inv.total) - float(inv.amount_paid)
        writer.writerow([
            inv.invoice_number, client.name if client else "", str(inv.issue_date), str(inv.due_date),
            f"{float(inv.subtotal):.2f}", f"{float(inv.tax_rate):.2f}", f"{float(inv.tax_amount):.2f}",
            f"{float(inv.total):.2f}", f"{float(inv.amount_paid):.2f}", f"{balance:.2f}",
            inv.currency, inv.status, str(inv.sent_at or ""), str(inv.paid_at or ""),
        ])
    output.seek(0)
...
def export_payments_csv(db: DbSession = Depends(get_db)):
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["Date", "Invoice Number", "Client", "Amount", "Payment Method", "Reference", "Notes"])
    for p in payments:
        inv = db.get(Invoice, p.invoice_id)
        client = db.get(Client, inv.client_id) if inv else None
        writer.writerow([
            str(p.payment_date), inv.invoice_number if inv else "", client.name if client else "",
            f"{float(p.amount):.2f}", p.payment_method or "", p.reference or "", p.notes or "",
        ])
    output.seek(0)
    return StreamingResponse(output, media_type="text/csv",
