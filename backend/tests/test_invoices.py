...
def _setup_client_with_sessions(client, num_sessions=2):
    r = client.post("/api/clients", json={
        "name": "Invoice Client", "currency": "CAD", "default_rate": 150.0, "payment_terms": 30,
    })
    cid = r.json()["id"]
    session_ids = []
...
def test_create_invoice_from_sessions(client):
    ...
    resp = client.post("/api/invoices", json={
        "client_id": cid, "session_ids": sids,
    })
...
def test_create_invoice_with_tax(client):
    ...
    resp = client.post("/api/invoices", json={
        "client_id": cid, "session_ids": sids, "tax_rate": 13.0,
    })
    data = resp.json()
...
def test_generate_invoices_for_all_unbilled(client):
    ...
    client.post("/api/sessions", json={
        "client_id": cid1, "date": "2025-08-21", "duration_minutes": 60, "hourly_rate": 100.0,
    })
    client.post("/api/sessions", json={
        "client_id": cid2, "date": "2025-08-22", "duration_minutes": 90, "hourly_rate": 200.0,
    })
...
def test_sequential_invoice_numbers(client):
    ...
    for i in range(3):
        r = client.post("/api/sessions", json={
            "client_id": cid, "date": f"2025-09-{i + 1:02d}",
