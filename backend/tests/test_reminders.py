def _create_invoice_with_reminders(client):
    r = client.post("/api/clients", json={"name": "Reminder Client", "payment_terms": 30})
    cid = r.json()["id"]
...
def test_payment_skips_pending_reminders(client):
    ...
    client.post("/api/payments", json={
        "invoice_id": inv_id, "amount": 150.0, "payment_date": "2025-09-01",
    })
    reminders = client.get(f"/api/reminders?invoice_id={inv_id}").json()
...
def test_filter_reminders_by_status(client):
    ...
    pending = client.get("/api/reminders?status=pending").json()
    sent = client.get("/api/reminders?status=sent").json()
    assert len(sent) == 1
