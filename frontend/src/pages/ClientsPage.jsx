...
const empty = { name: '', email: '', phone: '', company: '', currency: 'CAD', default_rate: '', payment_terms: 30, notes: '' };

export default function ClientsPage() {
  const { data: clients, loading, refetch } = useApi(() => api.getClients());
  ...
  const openEdit = (c) => { setForm({ ...c, default_rate: c.default_rate || '' }); setEditing(c.id); setModal(true); };

  const saveAction = useAction(
    useCallback(async (formData, editId) => {
      const data = { ...formData, default_rate: formData.default_rate ? Number(formData.default_rate) : null };
  ...
  const remove = async (id) => {
    if (!confirm('Delete this client? Their sessions and invoices will remain in the system.')) return;
    setDeletingId(id);
          ...
          <tbody>
            {clients?.map((c) => (
              <tr key={c.id}>
                ...
                <td>{c.email || '-'}</td>
                <td>{c.currency}</td>
                <td style={{ fontFamily: 'var(--font-heading)' }}>{c.default_rate ? `$${c.default_rate}/hr` : '-'}</td>
        ...
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" step="0.01" value={form.default_rate} onChange={e => setForm({ ...form, default_rate: e.target.value })} className="form-input" placeholder="e.g. 150" />
              <p className="text-xs mt-1" style={{ color: 'var(--color-text-tertiary)' }}>Auto-filled when logging sessions</p>
            </div>
            <div>
              <label className="form-label">Payment Due In (days)</label>
              <input type="number" value={form.payment_terms} onChange={e => setForm({ ...form, payment_terms: Number(e.target.value) })} className="form-input" />
              <p className="text-xs mt-1" style={{ color: 'var(--color-text-tertiary)' }}>How long after invoicing to expect payment</p>
            </div>
          </div>
          <div>
          ...
          >
            {saveAction.loading ? 'Saving...' : editing ? 'Update Client' : 'Create Client'}
          </button>
        </form>
      </Modal>
    </div>
