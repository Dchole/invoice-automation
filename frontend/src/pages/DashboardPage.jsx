...
export default function DashboardPage() {
  ...
  const {
    ...
    loading: l1,
    error: e1
  } = useApi(() => api.getDashboardSummary(currency), [currency]);
  const {
    data: aging,
    loading: l2,
    error: e2
  } = useApi(() => api.getDashboardAging(currency), [currency]);
  const {
    data: scores,
    loading: l3,
    error: e3
  } = useApi(() => api.getClientScores(currency), [currency]);
      ...
      >
        <div className="text-center">
          <p className="text-sm mb-2" style={{ color: "#b84c4c" }}>
            Failed to load dashboard: {anyError}
          </p>
          <button
        ...
        <div className="space-y-3 mb-8">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  {fmt(summary.total_overdue)} unpaid
                </span>
              </div>
              <span
                className="text-xs font-medium"
                style={{ color: "var(--color-status-red)", opacity: 0.7 }}
              >
                View &rarr;
              </span>
            </a>
            ...
            >
              <div className="flex items-center gap-3">
                <span
                  ...
                  style={{ backgroundColor: "var(--color-status-amber)" }}
                />
                <span
                  className="text-sm font-medium"
                  style={{ color: "var(--color-status-amber)" }}
                >
                  {summary.unbilled_sessions} session
                  {summary.unbilled_sessions !== 1 ? "s" : ""} ready to invoice
                  — {fmt(summary.unbilled_amount)}
                </span>
              </div>
              <span
                className="text-xs font-medium"
                style={{ color: "var(--color-status-amber)", opacity: 0.7 }}
              >
                Generate &rarr;
              </span>
            </a>
          ...
          {summary.draft_invoices > 0 && (
            <a
              ...
              onMouseEnter={e => {
                e.currentTarget.style.backgroundColor =
                  "rgba(69, 118, 169, 0.14)";
              }}
              onMouseLeave={e => {
                e.currentTarget.style.backgroundColor =
                  "rgba(69, 118, 169, 0.08)";
              }}
            >
              <div className="flex items-center gap-3">
                <span
                  className="w-2.5 h-2.5 rounded-full"
                  style={{ backgroundColor: "#4576a9" }}
                />
                <span
                  className="text-sm font-medium"
                  style={{ color: "#4576a9" }}
                >
                  {summary.draft_invoices} invoice
                  {summary.draft_invoices !== 1 ? "s" : ""} still in draft — not
                  sent to client yet
                </span>
              </div>
              <span
                className="text-xs font-medium"
                style={{ color: "#4576a9", opacity: 0.7 }}
              >
                Review &rarr;
              </span>
            </a>
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-5 stagger-children">
          ...
          <Card
            ...
            accent={
              summary.total_overdue > 0 ? "var(--color-status-red)" : undefined
            }
          />
          <Card
            title="Ready to Bill"
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          ...
          <Card
            ...
            accent={
              summary.collection_rate != null && summary.collection_rate >= 90
          ...
          <Card
            ...
            value={
              summary.avg_invoicing_days != null
                ? `${summary.avg_invoicing_days} days`
                : "—"
            }
            subtitle="Avg session to invoice"
            accent={
              summary.avg_invoicing_days != null &&
              summary.avg_invoicing_days <= 1
                ? "var(--color-status-green)"
                : summary.avg_invoicing_days != null &&
                    summary.avg_invoicing_days > 7
                  ? "var(--color-status-red)"
                  : undefined
            }
          />
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={agingData} barCategoryGap="25%">
              ...
              <Tooltip
                formatter={v => fmt(v)}
                {...CHART_TOOLTIP}
                cursor={{ fill: "rgba(200,135,95,0.06)" }}
              />
              <Bar
                dataKey="value"
          ...
          {statusData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                ...
                >
                  {statusData.map((d, i) => (
                    <Cell key={i} fill={PIE_COLORS[d.name] || "#7c7f86"} />
                  ))}
                </Pie>
      ...
      >
        ...
        {l4 ? (
          ...
          <p
            ...
            style={{ color: "var(--color-text-tertiary)" }}
          >
            No outstanding invoices to forecast
          </p>
        )}
        <p
          ...
          style={{ color: "var(--color-text-tertiary)" }}
        >
          Based on outstanding invoices, adjusted by each client's payment
          history
        </p>
      </div>
