...
export default function InvoicesPage() {
  ...
  const {
    ...
    error,
    refetch
  } = useApi(() => api.getInvoices(invoiceParams), [page, filter]);
  ...
  const clientUnbilled =
    unbilled?.filter(s => String(s.client_id) === createForm.client_id) || [];
  ...
  return (
    <div className="animate-fade-in">
            ...
            <tbody>
              {invoices.map(inv => (
                <tr key={inv.id}>
                  ...
                  >
                    {clientMap[inv.client_id]?.name || `#${inv.client_id}`}
                  </td>
      ...
      <Modal
        ...
        title="Create Invoices for Unbilled Work"
      >
        <p
        ...
        >
          ...
          sessions that haven't been billed yet, across{" "}
          <strong style={{ color: "var(--color-text-primary)" }}>
            {Object.keys(unbilledByClient).length}
          </strong>{" "}
          clients. Ready to create invoices?
        </p>
          ...
          {Object.entries(unbilledByClient).map(([cid, sessions]) => (
            ...
            >
              ...
              >
                ...
                {fmt(sessions.reduce((s, x) => s + x.amount, 0))}
              </span>
            </div>
          ))}
        </div>
        ...
        >
            ...
            >
              ...
              {clientUnbilled.map(s => (
                ...
                >
                  ...
                  />
                  {s.date} — {s.duration_minutes} min — {fmt(s.amount)}
          ...
          <div>
            ...
            <input
              ...
              onChange={e =>
                setCreateForm({ ...createForm, tax_rate: e.target.value })
              }
              className="form-input"
            />
          </div>
