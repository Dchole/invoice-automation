...
export default function PaymentsPage() {
  ...
  const { data: result, loading, refetch } = useApi(() => api.getPayments({ page, per_page: 25 }), [page]);
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          onClick={() => { setForm({ ...form, payment_date: new Date().toISOString().split('T')[0] }); setModal(true); }}
            ...
            {payments.map((p) => {
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td className="font-mono text-xs" style={{ color: 'var(--color-text-tertiary)' }}>{inv?.invoice_number || `#${p.invoice_id}`}</td>
                  ...
                  <td style={{ color: 'var(--color-text-tertiary)' }}>{p.reference || '-'}</td>
                </tr>
              );
            })}
            {payments.length === 0 && (
              <tr>
                <td colSpan={6} className="py-12! text-center" style={{ color: 'var(--color-text-tertiary)' }}>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Record a Payment You Received">
        <form onSubmit={save} className="space-y-4">
          <div>
            <label className="form-label">Which invoice was paid?</label>
            <select required value={form.invoice_id} onChange={e => prefillAmount(e.target.value)} className="form-input">
              <option value="">Select an invoice</option>
              {unpaidInvoices.map(i => (
                <option key={i.id} value={i.id}>{i.invoice_number} - {clientMap[i.client_id]} - {fmt(i.total - i.amount_paid)} remaining</option>
              ))}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="number" step="0.01" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} className="form-input" />
            </div>
            <div>
              <label className="form-label">Date</label>
              <input required type="date" value={form.payment_date} onChange={e => setForm({ ...form, payment_date: e.target.value })} className="form-input" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
            ...
            <div>
              ...
              <input value={form.reference} onChange={e => setForm({ ...form, reference: e.target.value })} className="form-input" placeholder="From your bank or payment service" />
            </div>
          </div>
          <div>
